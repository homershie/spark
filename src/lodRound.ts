...
export const LOD_SLICE_GROWTH = 1.5;
/** 片預算上限(ms)。 */
export const LOD_SLICE_MAX_MS = 200;
...
export function sliceBudget(
  ...
  if (sliceMs <= 0) return 0;
  if (slice === 0) return firstMs > 0 ? firstMs : sliceMs;
  return Math.min(LOD_SLICE_MAX_MS, sliceMs * LOD_SLICE_GROWTH ** slice);
}
